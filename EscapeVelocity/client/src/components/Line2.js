...
 */
    ...
    handleClick = (e) => {
        ...
        for (var i = 0; i < e.points.length; i++) {
            this.drawScatter(e.points[i].x, e.points[i].y)
        }
    ...
    drawScatter = (x, y) => {
        console.log(x)
        console.log(y)
        let sc = this.state.ukstate

        let sc1 = sc.filter(t => t.name == x).map(p => p.y)
        let sc2 = sc.filter(t => t.name == y).map(p => p.y)
        console.log("sc1", sc1)
        console.log("sc1", sc2)
        this.props.getScatter(sc1, sc2, x, y)
    }
    ...
    render() {
        ...
        //names
        var myarraynames = new Array(uk.length)
        for (var i = 0; i < uk.length; i++)
            myarraynames[i] = uk[i].name





        console.log('myarraynames', myarraynames)


        this.setState((prevState) => {
            if (JSON.stringify(prevState.ukstate) != JSON.stringify(uk)) {
                return { ukstate: uk }
            }
        })
        return (
            <Plot className='barchart ml-auto mr-auto' onClick={this.handleClick}
                data={[{
                    z: myarray,
                    x: myarraynames,
                    y: myarraynames,
